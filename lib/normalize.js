...
const createNormalizer = ({
  ...
  if (isUnlink) {
    if (isNotNull(dst)) throw new Error('Cannot set dst mode for unlink.')
    if (isNotNull(fmode)) throw new Error('Cannot set fmode mode for unlink.')
    if (isNotNull(dmode)) throw new Error('Cannot set dmode mode for unlink.')
    if (isNotNull(owner)) throw new Error('Cannot set owner for unlink.')
    if (isNotNull(group)) throw new Error('Cannot set group for unlink.')
  }

  if (isDir) {
    if (isNotNull(fmode)) throw new Error('Cannot set fmode mode for directory.')
  }

  if (isFile) {
    if (isNotNull(dmode)) throw new Error('Cannot set dmode mode for file.')
  }

  if (isSymlink) {
    if (isNotNull(fmode)) throw new Error('Cannot set fmode mode for symlink.')
    if (isNotNull(dmode)) throw new Error('Cannot set dmode mode for symlink.')
    if (isNotNull(owner)) throw new Error('Cannot set owner for symlink.')
    if (isNotNull(group)) throw new Error('Cannot set group for symlink.')
  }
...
module.exports = async ({hostname, root, packageChecker, ...manifest}) => {
  ...
  const targetsByType = [
    manifest.unlinks.map(x => ({...x, type: 'unlink'})),
  ...
  const [ unlinks, directories, files, symlinks ] =
    normalized.map(x => x.filter(isNotNull))

  return {unlinks, directories, files, symlinks}
}
