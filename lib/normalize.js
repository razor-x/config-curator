...
const arePackagesInstalled = async pkgs => {
...
const normalizeTarget = ({
  ...
  if (!arePackagesInstalled(pkgs)) return null
...
module.exports = async ({hostname, root, ...manifest}) => {
  const isNotNull = x => x !== null
  const normalizeTargets = x => x.map(normalizeTarget({hostname, root}))
